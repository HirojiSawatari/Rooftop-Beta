...
App({
  ...
  shareAppMessage: function (options) {
    var that = this;
    return {
      title: options.title || this.getAppTitle() || '',
  ...
  tapPageShareHandler: function (event) {
    ...
    let params = '';
    for (let i in this.globalData.appOptions.query){
      if (i != needStatistics && i != statisticsType){
        params += '&' + i + '=' + this.globalData.appOptions.query[i]
      }
    }
    this.sendRequest({
...
    return  s;
  ...
  globalData:{
    appId: '0ba1GCehMp',
        tabBarPagePathArr: '["/pages/index/index","/pages/find/find","/pages/me/me"]',
